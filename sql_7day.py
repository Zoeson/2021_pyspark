...
def get_weekly_data(sc,  date, hour):
    ...
    sql_date, sql_hour = get_new_time(now_time, -7 * 24)   # 一周前
    ...
    print('   SAVE DATA: ==')
    rdd_data.repartition(100).saveAsTextFile(output_path)
    print('     save finished consume time: {}'.format(datetime.datetime.now() - t))
